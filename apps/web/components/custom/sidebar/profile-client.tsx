...
export function ProfileClient({ data }: { data: SidebarData | null }) {
  ...
  const avatarUrl = data.avatarUrl?.startsWith('/') ? `${API_URL}${data.avatarUrl}` : data.avatarUrl
          ...
          <HoverCardTrigger asChild>
            <SidebarMenuButton
              ...
              className={cn(
                ...
                !data.hasActiveCampaign && 'opacity-50 grayscale pointer-events-none',
              )}
              disabled={!data.hasActiveCampaign}
            >
              <div className="flex aspect-square size-8 items-center justify-center rounded-lg bg-sidebar-primary text-sidebar-primary-foreground">
                <UserStar className="size-4" />
              </div>
              <div className="grid flex-1 text-left text-sm leading-tight">
                <span className="truncate font-medium">{data.ballotName}</span>
                <span className="truncate text-xs opacity-80">
                  {data.hasActiveCampaign
                    ? `${data.positionName} • ${data.partySlug}`
                    : 'Sem campanha ativa'}
                </span>
              </div>
            </SidebarMenuButton>
                ...
                <div className="flex flex-1 flex-col justify-center p-4 space-y-1.5 overflow-hidden relative">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[11px] font-bold text-primary/90 uppercase tracking-tighter">
                      {data.positionName} — {data.candidateNumber}
                    </p>
                  </div>
                  ...
                  <div className="grid gap-1.5 pt-2 text-[11px] text-muted-foreground">
                    <div className="flex items-start">
                      ...
                      <span className="text-[10px] opacity-70 leading-tight">{data.partyName}</span>
                    </div>
                    <div className="flex items-center font-medium">
                      <MapPinIcon className="mr-1.5 size-3 opacity-60 shrink-0" />
                      <span className="truncate text-foreground/80">
                        {data.municipalityName} - {data.stateAcronym}
                      </span>
                    </div>
                  </div>
